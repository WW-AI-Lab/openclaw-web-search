...
3. ✅ 第三阶段：适配 OpenClaw 2026.4.26+ 新接口，将 `providerAuthEnvVars` 迁移至 `setup.providers[].envVars`，消除延迟废弃警告。（已发布 v2026.4.29）
4. 第四阶段：增加豆包 / ARK 与智普搜索，继续补充响应归一化测试。
5. 第五阶段：抄象更多共享基础设施，兼容更多可接入的搜索服务。
6. 第六阶段：完善 Provider 选择策略、文档、示例与自动化发布流程。
